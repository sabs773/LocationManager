...
<body>
<form action = "SaveDuty.php" method="POST" id="framework_form">
...
<div class="container" style="width:600px;">
<div class="left">
<label>Assign Location :</label></div>
<div class="right">
<select name="Places" onchange="removeAll(); showUser(this.value); document.getElementById('LocationName').value=this.options[this.selectedIndex].text">
<?php $y= 0; foreach($place[0] as $rows){ $y++;?>
<option value="<?php echo $rows['LocationID'];?>">
<?php echo $rows['LocationName'];?>
</option>
<?php } ?>
</select></div>
</div><br></br>
...
<input type="text" id="note" name="note" placeholder="Additional notes" class="form-control" size="50" /></div>
</div></div>
...
&nbsp; &nbsp;&nbsp;
<button type="button" class="btn btn-danger" value="cancel" onclick="window.location.href='LocationView.php';">Cancel</button></div></center>
</div>  
  ...
  $.ajax({
    ...
    success: function(html) {
      ...
      for(var i=0; i< len; i++){
        ...
        var sel = document.getElementById('Block');

        // create new option element
        var opt = document.createElement('option');

        // create text node to add to option element (opt)
        opt.appendChild( document.createTextNode(test[i]));
...
//Remove All from DropBox
function removeAll(){
			selectBox = document.getElementById("Block");
...
//    success:function(data)
//    {
//     $('#cleaner option:selected').each(function(){
//      $(this).prop('selected', false);
//     });
//     $('#cleaner').multiselect('refresh');
//     alert("Sabrina");
//    }
//   });
//  });
